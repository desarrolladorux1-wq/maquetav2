...
(function () {
  ...
  const scriptGlobal = [...document.scripts].find(script => /scripts_generales\.js(?:\?|$)/.test(script.src));
  const rutaTemaPaulet = scriptGlobal ? new URL('tema_paulet.css?v=20260831-3', scriptGlobal.src).href : '/tema_paulet.css?v=20260831-3';
  ...
  window.aplicarTemaVisual = function (tema) {
    ...
    let enlace = document.getElementById('temaPauletGlobal');
    if (temaValido === 'paulet' && !enlace) {
      enlace = document.createElement('link');
      enlace.id = 'temaPauletGlobal';
      enlace.rel = 'stylesheet';
      enlace.href = rutaTemaPaulet;
      document.head.append(enlace);
    }
    if (enlace) enlace.disabled = temaValido !== 'paulet';
