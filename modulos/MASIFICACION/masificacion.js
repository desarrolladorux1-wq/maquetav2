(function () {
  ...
  let herramientaActiva=null,capaDibujo,puntosDibujo=[],centroCirculo=null,figuraTemporal=null;
  ...
  function cambiarVersionCartografica(version) {
    ...
    $('indicadorVersionMapa').textContent=version==='asbuilt'?'AS-BUILT · RED CONSTRUIDA':'PLANIFICACIÓN · ANTEPROYECTO';
    $('indicadorVersionMapa').classList.toggle('asbuilt',version==='asbuilt');
  ...
  function renderBarraProyectos(){
    ...
    ordenarProyectosAnidados(proyectos).forEach(({proyecto,nivel})=>{
      ...
      seleccionar.innerHTML=`<i style="--color-proyecto:${colores[proyecto.estado]||'#5b8fb0'}"></i><span><strong>${proyecto.nombre}</strong><small>${proyecto.codigo} · ${proyecto.estado}</small><em>Líder: ${proyecto.responsableLider||'Sin asignar'}</em><small>${nivel?`Subproyecto de ${ciudades.find(p=>p.codigo===proyecto.proyectoPadre)?.nombre||proyecto.proyectoPadre}`:proyecto.departamento}</small></span>`;
  ...
  function mostrarDetalle(p) {
    ...
    if($('panelCapas').hidden)alternarPanel($('botonCapas'),$('panelCapas'));
  ...
  function cerrarDibujo(e){
    if(herramientaActiva==='poligono'&&puntosDibujo.length>=3){capaDibujo.clearLayers();L.polygon(puntosDibujo,{pane:'dibujoPane',color:'#d98b24',fillColor:'#f2ad50',fillOpacity:.18,weight:3}).addTo(capaDibujo);mostrarSeleccion(registrosSeleccionables().filter(r=>puntoEnPoligono(r,puntosDibujo)),'Selección por polígono');puntosDibujo=[];figuraTemporal=null;}
    else if(herramientaActiva==='circulo'&&centroCirculo&&figuraTemporal){const radio=mapa.distance(centroCirculo,e.latlng);figuraTemporal.setRadius(radio);mostrarSeleccion(registrosSeleccionables().filter(r=>mapa.distance(centroCirculo,L.latLng(r.lat,r.lng))<=radio),'Selección por círculo');centroCirculo=null;figuraTemporal=null;}
  }
  ...
  function actualizar() {
    ...
    datos.forEach((p,i)=>{
      ...
      const icono=L.divIcon({className:'marcador-proyecto',html:`<span class="icono-proyecto-mapa ${versionCartograficaActiva==='asbuilt'?'asbuilt':'planificacion'}" style="--color-proyecto:${colorVersion}">${iconoProyecto}</span>`,iconSize:[40,40],iconAnchor:[20,20]});
      const punto=L.marker([p.lat,p.lng],{icon:icono}).bindTooltip(`${p.codigo} · ${p.nombre}`).on('click',e=>{
        if(['poligono','circulo'].includes(herramientaActiva)){clickDibujo(e);return;}
        subproyectoSeleccionado='';
        delete p.faseSeleccionada;
        mostrarDetalle(p);
  ...
  function cargarProyectoEnFormulario(proyecto) {
    ...
    cronogramaBorrador=structuredClone(proyecto.cronograma||[]);
  ...
  function obtenerDatosFormularioProyecto() {
    return {
      ...
      lat: proyectoEdicionSeleccionado?.lat ?? -10.6,
      lng: proyectoEdicionSeleccionado?.lng ?? -75.2,
  ...
  function crearFilaCronograma(actividad={}) {
    ...
    fila.innerHTML=`<td><input data-cronograma="codigo" value="${actividad.codigo||`ACT-${String(indice).padStart(2,'0')}`}"></td><td><input data-cronograma="actividad" value="${actividad.actividad||''}" placeholder="Ej. Elaborar expediente"></td><td><select data-cronograma="fase"><option>Anteproyecto</option><option>Proyecto</option><option>Construcción</option><option>Operación</option></select></td><td><input data-cronograma="inicio" type="date" value="${actividad.inicio||''}"></td><td><input data-cronograma="fin" type="date" value="${actividad.fin||''}"></td><td><input data-cronograma="predecesora" value="${actividad.predecesora||''}" placeholder="ACT-01"></td><td><input data-cronograma="sucesora" value="${actividad.sucesora||''}" placeholder="ACT-03"></td><td><button type="button" data-accion="eliminar-actividad" aria-label="Eliminar actividad">×</button></td>`;
  ...
  function leerCronogramaProyecto() {
    cronogramaBorrador=[...$('tablaCronogramaProyecto').querySelectorAll('tr')].map(fila=>Object.fromEntries([...fila.querySelectorAll('[data-cronograma]')].map(campo=>[campo.dataset.cronograma,campo.value.trim()]))).filter(actividad=>actividad.codigo||actividad.actividad);
  }
  ...
  function iniciar() {
    ...
    $('listaProyectosMapa')?.addEventListener('click',evento=>{
      ...
      if(accion){
        ...
        if(!proyecto)return;
        if(accion.dataset.accionBarra==='editar'){
