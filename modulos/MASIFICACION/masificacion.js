(function () {
  ...
  function obtenerDatosFormularioProyecto() {
    ...
    return {
      ...
      componente: $('proyectoComponente').value,
  ...
  function actualizarIndicadorAsistente(paso){document.querySelectorAll('#modalCrearProyecto [data-indicador-pasos] span').forEach((item,i)=>{item.classList.toggle('activo',i===paso-1);item.classList.toggle('completo',i<paso-1);});}
  function renderResumenFinalProyecto(){const m=metricasPlanificacion(),beneficiarios=$('tablaBeneficiariosProyecto').querySelectorAll('tr:not(.fila-vacia-beneficiarios)').length;$('contenidoResumenProyecto').innerHTML=`<div class="tarjetas-resumen-final"><article><small>Proyecto</small><strong>${escapePlanificacion($('proyectoCodigo').value||'Sin código')}</strong><span>${escapePlanificacion($('proyectoNombre').value||'Sin nombre')}</span></article><article><small>Ubicación</small><strong>${escapePlanificacion($('proyectoDepartamento').value||'Sin departamento')}</strong><span>${escapePlanificacion($('proyectoProvincia').value||'')} · ${escapePlanificacion($('proyectoDistrito').value||'')}</span></article><article><small>Planificación técnica</small><strong>${m.avance}% · ${m.resultado}</strong><span>${m.cumplidos} de ${m.total} requisitos cumplidos</span></article><article><small>Cronograma</small><strong>${cronogramaBorrador.length} actividades</strong><span>${escapePlanificacion($('proyectoFechaInicio').value||'Sin fecha')} — ${escapePlanificacion($('proyectoFechaFin').value||'Sin fecha')}</span></article><article><small>Beneficiarios</small><strong>${beneficiarios}</strong><span>registros cargados</span></article></div>`;}
  function mostrarPasoAsistenteProyecto(paso){pasoAsistenteProyecto=paso;const modal=$('modalCrearProyecto');if(!modal.open)modal.showModal();$('pasoDatosProyecto').hidden=paso!==1;$('pasoBeneficiariosProyecto').hidden=true;$('pasoResumenProyecto').hidden=paso!==4;$('anteriorAsistenteProyecto').hidden=paso===1;$('cancelarCrearProyecto').hidden=paso!==1;$('guardarBorradorProyecto').hidden=paso===4;$('continuarAsistenteProyecto').hidden=paso===4;$('guardarCrearProyecto').hidden=paso!==4;document.querySelector('.boton-subir-beneficiarios').hidden=false;actualizarIndicadorAsistente(paso);$('etiquetaCrearProyecto').textContent=`PASO ${paso} DE 4`;$('tituloCrearProyecto').textContent=paso===1?'Datos del proyecto':'Resumen final';$('descripcionCrearProyecto').textContent=paso===1?'Registre la información general, ubicación y responsables.':'Revise todos los datos antes de crear el proyecto.';if(paso===4)renderResumenFinalProyecto();}
  function abrirPlanificacionTecnica(proyecto){if(proyecto)planificacionBorrador=crearPlanificacionProyecto(proyecto.planificacionTecnica);else if(!planificacionBorrador)planificacionBorrador=crearPlanificacionProyecto();const codigo=proyecto?.codigo||$('proyectoCodigo').value||'Proyecto nuevo',nombre=proyecto?.nombre||$('proyectoNombre').value||'';$('subtituloPlanificacionTecnica').textContent=`${codigo}${nombre?' · '+nombre:''} · evaluación previa al inicio de la ejecución.`;renderPlanificacionTecnica();agregarFilasEjemploPlanificacion();$('modalPlanificacionTecnica').showModal();}
  ...
  function iniciar() {
    ...
    const abrirCrearProyecto=()=>{
      ...
      renderListaProyectosEdicion();
      limpiarFormularioProyecto();
      modalCrearProyecto.showModal();mostrarPasoAsistenteProyecto(1);
    };
    ...
    $('continuarAsistenteProyecto').addEventListener('click',()=>{if(pasoAsistenteProyecto===1){modalCrearProyecto.close();abrirPlanificacionTecnica(proyectoEdicionSeleccionado);}});
    $('anteriorAsistenteProyecto').addEventListener('click',()=>{if(pasoAsistenteProyecto===4){modalCrearProyecto.close();renderCronogramaProyecto();$('modalCronogramaProyecto').showModal();}});
    ...
    $('anteriorPlanificacionAsistente').addEventListener('click',()=>{$('modalPlanificacionTecnica').close();mostrarPasoAsistenteProyecto(1);});
    $('continuarPlanificacionAsistente').addEventListener('click',()=>{leerPlanificacionFormulario();$('modalPlanificacionTecnica').close();renderCronogramaProyecto();$('modalCronogramaProyecto').showModal();});
    $('anteriorCronogramaAsistente').addEventListener('click',()=>{$('modalCronogramaProyecto').close();abrirPlanificacionTecnica(proyectoEdicionSeleccionado);});
    $('continuarCronogramaAsistente').addEventListener('click',()=>{leerCronogramaProyecto();$('modalCronogramaProyecto').close();mostrarPasoAsistenteProyecto(4);});
    ...
    $('abrirMapaUbicacionProyecto').addEventListener('click',async()=>{origenMapaCrearProyecto=true;modalCrearProyecto.close();const selector=$('proyectoZonaObjetivo');selector.replaceChildren(new Option(`${$('proyectoCodigo').value||'BORRADOR'} · ${$('proyectoNombre').value||'Proyecto en creación'}`,'borrador'));$('nombreZonaProyecto').value=$('proyectoNombre').value?`Área de ${$('proyectoNombre').value}`:'Área de influencia';puntosPoligonoUbicacion=[];$('modalDibujoArea').showModal();iniciarMapaUbicacionProyecto();renderPoligonoUbicacion();try{await mostrarDepartamentosMapaProyecto();}catch(error){$('estadoZonaProyecto').textContent='No se pudieron cargar los límites territoriales del Perú.';}});
