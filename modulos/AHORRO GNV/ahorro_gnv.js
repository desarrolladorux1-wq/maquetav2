(()=>{
  ...
  function exportarResumen(){
    ...
    const graficas=[...document.querySelectorAll('#graficas .kpis-graficas-gnv,#graficas .tarjeta-grafica-gnv')].map(nodo=>nodo.outerHTML).join('');
    const estilos=[...document.querySelectorAll('link[rel="stylesheet"]')].map(link=>`<link rel="stylesheet" href="${link.href}">`).join('');
    ventana.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Gráficas Ahorro GNV</title>${estilos}<style>body{padding:24px;background:#fff!important}.reporte-graficas{max-width:1100px;margin:auto}.reporte-graficas>h1{margin:0 0 5px}.reporte-graficas>p{margin:0 0 18px;color:#65758a}@media print{body{padding:0}.tarjeta-grafica-gnv{break-inside:avoid;box-shadow:none}.rejilla-graficas-gnv{display:block}.tarjeta-grafica-gnv{margin-bottom:12px}}</style></head><body><main class="reporte-graficas"><h1>AHORRO GNV · Gráficas</h1><p>${lista.length} registros según los filtros seleccionados</p>${graficas}</main><script>window.onload=()=>setTimeout(()=>window.print(),350)<\/script></body></html>`);ventana.document.close();
  }
