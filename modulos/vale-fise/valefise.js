(function () {
  ...
  let arrastre=null;
  elementos.barraHerramientas.addEventListener('pointerdown',function(evento){
    const boton=evento.target.closest('button');
    if(!boton||boton.dataset.herramienta!=='mover')return;
    elementos.barraHerramientas.classList.add('movible');
    boton.classList.add('activo');
    const rect=elementos.grupoHerramientas.getBoundingClientRect();
    elementos.grupoHerramientas.style.position='fixed';
    elementos.grupoHerramientas.style.left=`${rect.left}px`;
    elementos.grupoHerramientas.style.top=`${rect.top}px`;
    elementos.grupoHerramientas.style.right='auto';
    arrastre={x:evento.clientX-rect.left,y:evento.clientY-rect.top};
    elementos.grupoHerramientas.setPointerCapture(evento.pointerId);
    evento.preventDefault();
  });
  elementos.barraHerramientas.addEventListener('pointermove',function(evento){if(!arrastre)return;elementos.grupoHerramientas.style.left=`${Math.max(0,Math.min(innerWidth-elementos.grupoHerramientas.offsetWidth,evento.clientX-arrastre.x))}px`;elementos.grupoHerramientas.style.top=`${Math.max(0,Math.min(innerHeight-elementos.grupoHerramientas.offsetHeight,evento.clientY-arrastre.y))}px`;});
  elementos.barraHerramientas.addEventListener('pointerup',function(){arrastre=null;});
